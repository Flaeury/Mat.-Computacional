...
def minimos_quadrados_linear(num_pontos, pontos):
    ...
    a1 = (num_pontos * somatorio_xy - somatorio_x * somatorio_y) / \
        (num_pontos * somatorio_x2 - somatorio_x**2)
    ...
    desvio_padrao = (
        sum(residual**2 for residual in residuos) / num_pontos)**0.5
...
def calcular_regressao_quadratica(num_pontos, pontos):
    ...
    # Calcula os valores preditos y_pred
    y_pred = [(coeficientes[0] + coeficientes[1] *
               xi + coeficientes[2] * xi**2) for xi in x]
    ...
    # Calcula a raiz da média quadrática dos resíduos
    desvio_padrao = (
        sum(residual**2 for residual in residuos) / num_pontos) ** (1/2)
...
def minimos_quadrados_exponencial(num_pontos, pontos):
    ...
    coeficientes_log, desvio_padrao = minimos_quadrados_linear(
        num_pontos, novos_pontos)
