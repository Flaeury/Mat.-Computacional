...
def metodo_newton_raphson(funcao, derivada_funcao, x0, tol=0.00000001, max_iter=10000000):
    ...
    raise ValueError(
        "O método NAO convergiu dentro do número máximo de iterações.")
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"A raiz encontrada: x = {raiz:.6f} com {iteracoes} iterações.")

        # Teste para verificar a solução
        f_raiz = funcao(raiz)
        print(f"f({raiz:.6f}) = {f_raiz:.6e}, próximo de zero.")
