...
def eliminacao_de_gauss(matriz):
    ...
    for i in range(min(linhas, colunas - 1)):  # -1 para ignorar o termo independente na última coluna
...
def substituicao_retroativa(matriz_escalonada):
    ...
    for i in range(linhas - 1, -1, -1):  # Começa da última linha
        soma = sum(matriz_escalonada[i, j] * x[j] for j in range(i + 1, colunas - 1))
...
if __name__ == "__main__":
    ...
    for i in range(linhas):
        ...
        resultado = sum(matriz_escalonada[i, j] * matriz_escalonada[j, -1] for j in range(colunas - 1))
        ...
        print(f"  Diferença: {abs(resultado - matriz_escalonada[i, -1]):.6f}\n")
